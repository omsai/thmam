...
sim1.times.bbz.mov <- function(s, lam1, lam2, p) {
    ...
    tt <- NULL
    while (TRUE) {
        tnew <- rexp(1, rate=lam1)
        tsum <- tsum + tnew
        tt <- c(tt, tsum)
        if (tsum > s) break
        ind <- rbinom(1, size=1, prob=p) ## the prob of choosing lam2[1] is p
        tnew <- rexp(1, rate=lam2[1])*ind + rexp(1, rate=lam2[2])*(1-ind)
        tsum <- tsum + tnew
        tt <- c(tt, tsum)
        if (tsum > s) break
    }
...
sim1.times.bbz.sta <- function(s, lam1, lam2, p) {
    ...
    tt <- NULL
    while (TRUE) {
        ind <- rbinom(1, size=1, prob=p) ## the prob of choosing lam2 is p
        tnew <- rexp(1, rate=lam2[1])*ind + rexp(1, rate=lam2[2])*(1-ind)
        tsum <- tsum + tnew
        tt <- c(tt, tsum)
        if (tsum > s) break
...
##' dat <- rMovResSta(time=tgrid, lamM=4, lamR=0.04, lamS=0.2,
